...
from app.graph.state import AgentState, show_agent_reasoning
from app.utils.progress import progress
from app.utils.api_key import get_api_key_from_state
...
def valuation_analyst_agent(state: AgentState, agent_id: str = "valuation_analyst_agent"):
    ...
    api_key = get_api_key_from_state(state, "EODHD_API_KEY")
    ...
    for ticker in tickers:
        ...
        if hasattr(li_curr, "netWorkingCapital") and hasattr(li_prev, "netWorkingCapital"):
            wc_curr = li_curr.netWorkingCapital
            wc_prev = li_prev.netWorkingCapital
        else:
            wc_curr = None
            wc_prev = None
        ...
        # Get values from LineItem or fallback to FinancialMetrics
        net_income = getattr(li_curr, "netIncome", None) or getattr(most_recent_metrics, "net_income", 0)
        depreciation = getattr(li_curr, "depreciation", getattr(li_curr, "depreciationAndAmortization", 0))
        capex = getattr(li_curr, "capitalExpenditures", 0)

        if not isinstance(capex, (int, float)):
            capex = 0

        # Owner Earnings
        owner_earnings_val = calculate_owner_earnings_value(
        ...
        wacc = calculate_wacc(
            ...
            total_debt=getattr(li_curr, 'totalDebt', None),
            cash=getattr(li_curr, 'cashAndEquivalents', None),
        ...
        for li in line_items:
            if hasattr(li, 'freeCashFlow') and li.freeCashFlow is not None:
                fcf_history.append(li.freeCashFlow)
        ...
        method_values = {
            ...
            "owner_earnings": {"value": owner_earnings_val, "weight": 0.35},
