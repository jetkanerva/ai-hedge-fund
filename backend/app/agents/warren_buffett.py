...
from app.tools.api import get_financial_metrics, get_market_cap, search_line_items
...
def calculate_owner_earnings(financial_line_items: list) -> dict[str, any]:
    ...
    # Core components
    net_income = getattr(latest, "netIncome", getattr(latest, "net_income", None))
    depreciation = getattr(latest, "depreciation", getattr(latest, "depreciation_and_amortization", None))
    capex = getattr(latest, "capitalExpenditures", getattr(latest, "capital_expenditure", None))
    ...
    if len(financial_line_items) >= 2:
        try:
            wc_current = getattr(latest, "netWorkingCapital", None)

            previous = financial_line_items[1]
            wc_previous = getattr(previous, "netWorkingCapital", None)

            if wc_current is not None and wc_previous is not None:
    ...
    if len(historical_earnings) >= 3:
        ...
        if oldest_earnings > 0:
            if latest_earnings > 0:
                ...
                historical_growth = (latest_earnings - oldest_earnings) / (oldest_earnings * years)
